...
MultNom.removal.multmeth.lambda.mcmc<-function(ymat,gbe,gtypes,capnames,Areaper,Xd,pstart,sigla=1,siglb=1,sigBa=1,sigBb=1,
  ...
  for(l in 1:n.mcmc){
    ...
    if(length(dupind)>0){
      ...
      lambda1 <- lamd[dupindno1-1]*(nt[(dupindno1-1)]-ymatcum[dupindno1-1,passes])
      lambda2 <- lamd[dupindnolast]
      lambda3 <-

      nMHratio=exp(
        (sum(
          sapply(
            dupind,function(x)dmultinom(ynmat.star[x,],prob=pipn[x,],log=TRUE)))+
                      rowSums(cbind(
                        dpois(n.star[dupindno1],lambda1,log=TRUE)[match(dupind,dupindno1)],
                        dpois(nt[dupindnolast+1],lambda2*(n.star[(dupindnolast)]-ymatcum[dupindnolast,passes]),log=TRUE)[match(dupind,dupindnolast)],
                     ...
                     (sum(sapply(dupind,function(x)dmultinom(ynmat[x,],prob=pipn[x,],log=TRUE)))+
                        rowSums(cbind(dpois(nt[dupindno1],lambda1,log=TRUE)[match(dupind,dupindno1)],
                                      dpois(nt[dupindnolast+1],lambda2*(nt[(dupindnolast)]-ymatcum[dupindnolast,passes]),log=TRUE)[match(dupind,dupindnolast)],
